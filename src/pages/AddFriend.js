...
export default function AddFriend({ user, setFollowed }) {
    ...
    async function fetchUsers() {
      ...
      querySnapshot.forEach(async (doc) => {
        if (doc.data().User_id !== user.uid) {
          ...
          let url = "";
          try {
            url = await getDownloadURL(ref(storage, doc.data().Picture_id));
          } catch {
            url = await getDownloadURL(ref(storage, "default.png"));
          }
          optionVals.push({
            value: doc.data().Name,
            id: doc.data().User_id,
  ...
  const Follow = async () => {
    ...
    let new_obj = {};
    const docSnap = await getDoc(doc(db, "Profile", searchUser.id));
    const storage = getStorage();
    ...
    <Flex align="center" direction="column">
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalCloseButton />
          <ModalBody>
            <Box>Do you want to follow {searchUser.user_id} ?</Box>
          </ModalBody>
          <ModalFooter>
            <Button
              bg="primary.3200"
