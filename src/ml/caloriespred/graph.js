import {auth, collection, db, doc, getDoc, getDocs, query, where} from "../../firebase/firebase.js";
...
(async () => {
    ...
    const postQuery = query(collection(db, "test"), where("usr", "==", example), where("timestamp", ">=", target));
    let records = []
    ...
    getDocs(postQuery).then((querySnapshot) => {
        querySnapshot.forEach((activity) => {
            ...
            records.push({calories:calories(
                post["type"],
